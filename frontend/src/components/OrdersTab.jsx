    ...
    <div className='space-y-3'>
      {adminOrders.map((order)=><div key={order._id} className='rounded-xl border border-white/10 bg-white/5 p-4 text-white'>
        ...
        <div className='mt-3 flex gap-2'>
          <button className='rounded bg-green-600 px-3 py-1' onClick={async ()=>{await approveOrder(order._id); fetchAdminOrders();}}>Approve</button>
          <button className='rounded bg-red-600 px-3 py-1' onClick={async ()=>{await rejectOrder(order._id); fetchAdminOrders();}}>Reject</button>
        </div>
      </div>)}
    </div>
  );
